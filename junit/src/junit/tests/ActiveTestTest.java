...
public class ActiveTestTest extends TestCase {
	...
	public void testActiveTest() {		
		...
		test.run(result);
		junitAssertEquals(100, result.runCount());
		junitAssertEquals(0, result.failureCount());
		junitAssertEquals(0, result.errorCount());
	}
	...
	public void testActiveRepeatedTest() {		
		...
		TestResult result= new TestResult();
		test.run(result);
		junitAssertEquals(500, result.runCount());
		junitAssertEquals(0, result.failureCount());
		junitAssertEquals(0, result.errorCount());
	}
	...
	public void testActiveRepeatedTest0() {		
		...
		TestResult result= new TestResult();
		test.run(result);
		junitAssertEquals(0, result.runCount());
		junitAssertEquals(0, result.failureCount());
		junitAssertEquals(0, result.errorCount());
	}
